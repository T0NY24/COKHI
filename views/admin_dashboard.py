...
def admin_dashboard(page: ft.Page):
    ...
    users_table = ft.DataTable(
        ...
        rows=[
            ft.DataRow(
                cells=[
                    ft.DataCell(ft.Text(user.get("Nombre", "Sin Nombre"), size=14, color="#6D4318")),
                    ft.DataCell(ft.Text(user.get("CorreoElectronico", "Sin Email"), size=14, color="#25523E")),
                    ft.DataCell(ft.IconButton(icon=ft.icons.DELETE, icon_color="red600", on_click=delete_user, data=user.get("id")))
                ]
            ) for user in users
        ]
    )
    ...
    caregivers_table = ft.DataTable(
        ...
        rows=[
            ft.DataRow(
                cells=[
                    ft.DataCell(ft.Text(caregiver.get("Nombre", "Sin Nombre"), size=14, color="#6D4318")),
                    ft.DataCell(ft.Text(caregiver.get("Cedula", "Sin Cédula"), size=14, color="#25523E")),
                    ft.DataCell(ft.Text(caregiver.get("Email", "Sin Email"), size=14, color="#25523E")),
                    ft.DataCell(ft.Text(caregiver.get("Telefono", "Sin Teléfono"), size=14, color="#25523E")),
                    ft.DataCell(ft.IconButton(icon=ft.icons.DELETE, icon_color="red600", on_click=delete_caregiver, data=caregiver.get("id")))
                ]
    ...
    page.add(
        ft.Column([
            ft.Container(
                content=ft.Text("📋 Lista de Usuarios", size=24, weight="bold", color="#6D4318"),
                padding=10
            ),
            users_table,
            ft.Container(
                content=ft.Text("📋 Lista de Cuidadores", size=24, weight="bold", color="#6D4318"),
                padding=10
            ),
            caregivers_table
