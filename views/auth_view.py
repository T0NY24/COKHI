...
def login_view(page):
    ...
    login_button = ft.ElevatedButton(
        "Iniciar Sesión", bgcolor=PRIMARY_COLOR, color="white", on_click=handle_login
    )
    signup_button = ft.TextButton(
        "¿No tienes cuenta? Regístrate.", on_click=lambda _: page.go("/register")
    )
...
def signup_view(page):
    ...
    nombre_input = ft.TextField(
        label="Nombre", hint_text="Ingresa tu nombre", border_color=PRIMARY_COLOR
    )
    apellido_input = ft.TextField(
        label="Apellido", hint_text="Ingresa tu apellido", border_color=PRIMARY_COLOR
    )
    telefono_input = ft.TextField(
        label="Teléfono", hint_text="Ingresa tu teléfono", border_color=PRIMARY_COLOR
    )
    ciudad_input = ft.TextField(
        label="Ciudad", hint_text="Ingresa tu ciudad", border_color=PRIMARY_COLOR
    )
    error_text = ft.Text("", color="red")
    ...
    signup_button = ft.ElevatedButton(
        "Registrarse", bgcolor=PRIMARY_COLOR, color="white", on_click=handle_signup
    )
    login_button = ft.TextButton(
        "¿Ya tienes una cuenta? Inicia Sesión.", on_click=lambda _: page.go("/")
    )
...
def cuidador_signup_view(page):
    ...
    cedula_input = ft.TextField(
        label="Cédula", hint_text="Ingresa tu cédula", border_color=PRIMARY_COLOR
    )
    telefono_input = ft.TextField(
        label="Teléfono", hint_text="Ingresa tu teléfono", border_color=PRIMARY_COLOR
    )
